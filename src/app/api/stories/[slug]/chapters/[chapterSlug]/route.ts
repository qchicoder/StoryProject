...
export async function GET(
  ...
  try {
    ...
    } else if (!isNaN(parseInt(chapterSlug))) {
      chapterNum = parseInt(chapterSlug);
    }
    ...
    const chapter = await prisma.chapter.findFirst({
      where: {
        ...
        OR: [
          { slug: chapterSlug },
          ...(chapterNum !== null ? [{ chapterNumber: chapterNum }] : []),
        ],
      },
      include: {
        story: {
    ...
    return NextResponse.json(
      { success: false, message: 'Lỗi khi tải chi tiết chương: ' + error.message },
      { status: 500 }
    );
  }
}
